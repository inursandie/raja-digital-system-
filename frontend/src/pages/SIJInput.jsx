...
import { FileText, Printer, RotateCcw, CheckCircle2, Search, CalendarDays, X, User } from 'lucide-react';
...
export default function SIJInput() {
  ...
  const [form, setForm] = useState({ driver_id: '', sheets: '5', qris_ref: '', date: null });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(`${API}/sij`, payload, { headers: getAuthHeader() });
      setResult(res.data);
                ...
                <div>
                  ...
                  {selectedDriver && (
                    ...
                    >
                      ...
                      <span className="text-zinc-500 font-mono">{selectedDriver.plate}</span>
                    </motion.div>
                  )}
                </div>
                    ...
                    <PopoverTrigger asChild>
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <span className={form.date ? 'text-zinc-100' : 'text-zinc-500'}>
                            {form.date ? format(form.date, 'EEEE, d MMMM yyyy', { locale: id }) : 'Hari ini (default)'}
                          </span>
                        </div>
                        {form.date && (
                          <button
                            type="button"
                            onClick={(e) => { e.stopPropagation(); setForm(f => ({ ...f, date: null })); }}
                  ...
                  <div className="flex gap-2">
                    {[1,2,3,4,5,6,7].map(n => (
                      <button
                        key={n}
                        type="button"
                        onClick={() => setForm(f => ({ ...f, sheets: String(n) }))}
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    onClick={() => printReceiptBrowser(result, result.driver_name)}
                ...
                <p className="text-xs text-zinc-500">
                  * File ESC/POS (.bin) dapat dikirim ke thermal printer 58mm melalui aplikasi printer atau command line
                </p>
              </div>
