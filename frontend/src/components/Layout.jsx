    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-6 py-6 border-b border-zinc-800/50">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-black text-white text-lg leading-tight" style={{ fontFamily: 'Chivo, sans-serif' }}>
              RAJA
            </div>
            <div className="text-zinc-500 text-xs font-mono">Digital System v1.0</div>
          </div>
        </div>
      </div>
      ...
      <div className="px-3 py-4 border-t border-zinc-800/50">
        <div className="px-3 py-3 rounded-lg bg-zinc-900/50 mb-2">
          ...
          <div className="flex items-center gap-2 mt-0.5">
            <span className="text-xs text-zinc-500">{ROLE_LABELS[user?.role]}</span>
            {user?.shift && (
              <>
                <span className="text-zinc-700">·</span>
                <span className={`text-xs font-mono ${SHIFT_COLORS[user?.shift] || 'text-zinc-400'}`}>
                  {user?.shift}
                </span>
              </>
            )}
    ...
    <div className="flex h-screen bg-zinc-950 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="lg:hidden flex items-center justify-between px-4 py-3 border-b border-zinc-800/50 bg-zinc-900/30">
          ...
          <div className="font-black text-amber-500 text-base" style={{ fontFamily: 'Chivo, sans-serif' }}>
            RAJA
          </div>
          <div className="text-xs font-mono text-zinc-500">{user?.shift || 'SuperAdmin'}</div>
        </header>
